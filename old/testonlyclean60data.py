...
data.to_csv (r'C:\Users\OhoMindZa\Documents\jan-pro\part-ml\dlspot-code\setdate_clean.csv', index = None, header=True)
...
df = pd.DataFrame(data_old, columns= ['Humidity',	 'Temperature',	'Object(*C)', 'CO2PWM(ppm)', 'CO2Analog(ppm)','isGood' ])
...
inputs=df.drop('labeled', axis='columns')  #not have xbar sd and model
input_all = df_filter.drop('labeled', axis='columns')  #featuredata_new
...
data_norm = normalize(data)  #### edit here to change data to norm

# df_n =pd.DataFrame(data_norm, columns=   ['Temperature'	,'Humidity'	,'Object(*C)','CO2Analog(ppm)',
#                         'CO2PWM(ppm)',		'Xbar_Humidity', 'Xbar_co2analog',	'SD_Humidity',
#                         'SD_co2analog',	'Mode_Humidity'	,	'Mode_co2analog',	'labeled'])
df_n =pd.DataFrame(data_norm, columns=   ['Temperature'	,'Humidity'	,'Object(*C)','CO2Analog(ppm)',
                        'CO2PWM(ppm)',		'Xbar_Humidity', 'Xbar_co2analog',	'SD_Humidity',
                        'SD_co2analog',		'labeled'])

input_norm = df_n.drop('labeled', axis='columns')
...
inputs.drop('isGood', axis='columns',inplace=True)   #drop ตัวที่ไม่ใช้ออกไปอีก
input_all.drop('isGood', axis='columns',inplace=True)   #drop ตัวที่ไม่ใช้ออกไปอีก
...
# writer = pd.ExcelWriter('output_cur.xlsx')
# target.to_excel(writer,'labeled_old')
# target_filter.to_excel(writer,'labeled_new')
# inputs.to_excel(writer,'feature_old')
# input_all.to_excel(writer,'feature_new')
# # inputs2.to_excel(writer,'featuredata_without_tmp')
# input_norm.to_excel(writer,'feature_withnorm')
# writer.save()


writer = pd.ExcelWriter('output_alldata_clean.xlsx')
target_filter.to_excel(writer,'labeled')
target.to_excel(writer,'labeled_old')
...
# X_train_norm, X_test_norm, Y_train_norm, Y_test_norm = train_test_split(input_norm,target_filter,test_size=0.2,random_state=0) #all  norm and have tmp
...
# X_train_n, X_test_n, Y_train_n, Y_test_n =train_test_split(input_all,target_filter,test_size=0.2,random_state=0) #all with tmp new with xbar sd mode


# # X_train1, X_test1, Y_train1, Y_test1 =train_test_split(inputs2,target_filter,test_size=0.2,random_state=0) # all with out tmp
...
# SVR()
# svmmodel_n = clf_n.predict(X_test_n)
# svmscore_n = clf_n.score(X_train_n,Y_train_n)
...
# y_model_n = model_new.predict(X_test_n)
# # ###########Knn ##########
...
# # ##########Knn new ##########
# modelknn_n = KNeighborsClassifier(n_neighbors=5)
# modelknn_n.fit(X_train_n,Y_train_n)
# knn_score_n = modelknn_n.score(X_train_n,Y_train_n)
# answer_n = modelknn_n.predict(X_test_n)
# # print("Knn score : ", knn_score)
# #############Gau wotmp#############
# # model1= GaussianNB()
...
# #############Logistic Regress with new############ #เป็นวิธีการที่นิยมใช้เพื่อจำแนกประเภทข้อมูลหรือสิ่งของบางอย่างออกเป็นสองกลุ่ม แบบnorm
# logistic_regression_new= LogisticRegression()
# logistic_regression_new.fit(X_train_n,Y_train_n)
# y_pred=logistic_regression_new.predict(X_test_n)
...
# acc_new = get_score(model_new, X_train_n, X_test_n, Y_train_n, Y_test_n ) #Gau new
...
# acc_svm_n = get_score(clf_n,X_train_n,X_test_n,Y_train_n,Y_test_n) #use SVM new dataset

# acc_knn = get_score(modelknn,X_train_o,X_test_o,Y_train_o,Y_test_o) #use KNN
# acc_knn_n = get_score(modelknn_n,X_train_n,X_test_n,Y_train_n,Y_test_n) #use KNN new

# acc_logistic = get_score(logistic_regression,X_train_o,X_test_o,Y_train_o,Y_test_o)
# acc_logistic_new = get_score(logistic_regression_new,X_train_n,X_test_n,Y_train_n,Y_test_n)
...
# print('')
# print('')
# print('ค่า Accuracy with GaussianNB new data :',acc_new)
# print('')
# # print('ค่า Accuracy with GaussianNB โดยที่ไม่มีปัจจัยภายนอก :',acc_wotemp)
# print('')
# print('ค่า Accuracy with SVM old :',acc_svm)
# print('')
# print('ค่า Accuracy with SVM (&norm) :',acc_svm_norm)
# print('')
# print('ค่า Accuracy with SVM new :',acc_svm_n)
# print('')
# print('ค่า Accuracy with KNN old :',acc_knn)
# print('')
# print('ค่า Accuracy with KNN new :',acc_knn_n)
# print('')
...
# print('')
# print('ค่า Accuracy with logistic regress new :',acc_logistic_new)
# print('')
